...
wat <- c("Inloop (koffie)", "Sessies Ronde 1", "Pauze", "Sessies Ronde 2", "lunch", "Plenaire Sessie (keynotes)", "Pauze"," Sessies Ronde 3", "Borrel")
...
reactable(test2, 
  ...
  # Give rows a pointer cursor to indicate that they're clickable
    ...
    samenvatting = colDef(show = TRUE, html = TRUE, style = cell_style(test2, font_size = "10px") )# overrides the default
  )
...
test2 <- test %>% 
  mutate(sessie = panel,
         ...
         titel = titel_paper) %>%
  select(c("ronde", "sessie", "auteur", "coauteurs", "taal", "titel", "samenvatting"))
