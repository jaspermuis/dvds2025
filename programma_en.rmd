...
The program was last updated on **`r format(Sys.time(), format="%d %B %Y")`** om **`r substr(Sys.time(), 12,16)`**
...
test3 <- test %>% 
  mutate(Presenter = naam,
         ...
         Language = ifelse(Taal == "E", "English", Taal),
         Title = titel_paper) %>%
  select(c("Ronde", "Sessie", "Zaal", "Presenter", "coauteurs", "Language", "Title", "samenvatting_paper")) %>%
  arrange(Sessie)
