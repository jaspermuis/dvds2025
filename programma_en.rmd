...
-   Het The overview is currently sorted by Session (A, B, C, etc.). By clicking on the column headers, you can sort by, for example, author or round. staat nu gesorteerd op Sessie (A, B, C etc.). Door op de kolomnamen te klikken kunt u zelf sorteren op bijvoorbeeld `auteur` of `ronde`.\
-   Door Click the arrow to read the presentation abstracts. op het pijltje te klikken kunt u de samenvattingen van de presentaties lezen.
...
test3 <- test %>% 
  mutate(Presenter = naam,
         ...
         Titel = titel_paper) %>%
  select(c("Ronde", "Sessie", "Zaal", "Presenter", "coauteurs", "Taal", "Titel", "samenvatting_paper")) %>%
  arrange(Sessie)
